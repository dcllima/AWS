...
sc = SparkSession.Builder().master("yarn").appName("BalancoTeste").enableHiveSupport().getOrCreate()
...
a = spark.read.csv( \
    "s3://ve-datalake-stage-vg-dev/gene/contratos/comercial/tmp_aditivos/", \
    schema=aditivosSchema)
dt_ref = a.agg({"dt_ref": "max"}).collect()[0]
print(dt_ref["max(dt_ref)"])

out = a.filter(a.dt_ref == dt_ref["max(dt_ref)"])

out.write.mode("overwrite").saveAsTable("aditivo")
