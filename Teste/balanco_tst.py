...
if __name__ == "__main__":
	...
	vwContratosPropostas = spark.sql("SELECT A.Id AS ID_ADITIVO, \
	       									...
	       									CASE \
	           									WHEN C.TIPOPERIODOVIGENCIA = "+tipoPeriodoVigenciaContrCont+" THEN 'Curto Prazo' \
	           									ELSE 'Longo Prazo' \
	       									END AS EXTENSAO, \
	       									CT.Nome AS CLASSIFICACAO, \
	       									C.CodigoReferencia, \
	...
	vwContratos = spark.sql("SELECT ID_ADITIVO, \
								...
								JOIN FC_GET_NATUREZA B ON A.ID_ADITIVO = B.IDADITIVO \
								JOIN FC_GET_SERIE_REAJUSTE C ON A.ID_ADITIVO = C.ID \
								JOIN FC_GET_COMPRADOR_VENDEDOR D ON A.CodigoReferencia = D.CodigoReferencia \
								AND (CASE WHEN B.Natureza = 'Serviços' THEN 'CS' ELSE 'C' END) = D.TIPO\
								JOIN FC_GET_COMPRADOR_VENDEDOR E ON A.CodigoReferencia = E.CodigoReferencia \
								AND (CASE WHEN B.Natureza = 'Serviços' THEN 'VS' ELSE 'V' END) = E.TIPO \
								JOIN FC_GET_RECORRENCIA_REAJUSTE F ON A.ID_ADITIVO = F.ID\
								JOIN FC_GET_DIABASE_REAJUSTE G ON A.ID_ADITIVO = G.ID")
	...
	vwBalancoServicosStep2PrecoStep2 = spark.sql("SELECT DISTINCT NVL(TO_DATE(substr(AA.InicioValidade,0,10), 'yyyy-MM-dd'),TO_DATE(substr(B.DataInicioVigencia,0,10), 'yyyy-MM-dd')) AS INICIOVALIDADE, \
						   ...
						   WHERE AA.IDTIPO = 6 \
						   AND B.DtRef >= NVL(TO_DATE(substr(AA.InicioValidade,0,10), 'yyyy-MM-dd'),B.DataInicioVigencia) AND B.DtRef < NVL(TO_DATE(substr(AA.InicioValidade,0,10), 'yyyy-MM-dd'),ADD_MONTHS(B.DataFimVigencia,1))")
	...
	resultUnionStep1 = tmpBalancoServicosfinalStep1.union(tmpOutrosContratos.union(tmpLiquidacao.union(tmpModulacao)))

	resultUnionStep2 = tmpEnergiaEntregueFinal.union(tmpEnergiaContratadaSazoFinal)

	resultUnion = resultUnionStep2.union(resultUnionStep1)
	...
	TbFinalStep4 = spark.sql("SELECT A.DTA_REFERENCIA, \
			                  ...
			                  LEFT JOIN (SELECT MAX(DTA_FIM_REF) AS MAX_DATA AS DCIDE FROM DCIDE) AS B ON (A.DCIDE = B.DCIDE)")
	...
	TbFinalStep6 = TbFinalStep5.filter("DTA_REF is not null")

	TbFinalStep7 = fc_Pld_Submercado(TbFinalStep6,TbFinalStep6.Energia,TbFinalStep6.DTA_REF, TbFinalStep6.Periodo,TbFinalStep6.C_M0,TbFinalStep6.C_M1,TbFinalStep6.C_M2,TbFinalStep6.C_M3,TbFinalStep6.C_A0,TbFinalStep6.C_A1,TbFinalStep6.C_A2,TbFinalStep6.C_A3,TbFinalStep6.C_A4,TbFinalStep6.I_M0,TbFinalStep6.I_M1,TbFinalStep6.I_M2,TbFinalStep6.I_M3,TbFinalStep6.I_A0,TbFinalStep6.I_A1,TbFinalStep6.I_A2,TbFinalStep6.I_A3,TbFinalStep6.I_A4)

	TbFinalStep8 = TbFinalStep7.coalesce(50)

	TbFinalStep8.createOrReplaceTempView("TAB_FINAL_STEP8")

	TbFinalStep9Ini = spark.sql("SELECT A.MAX_DATA, \
										A.Periodo, \
										MAX(DT_INI.DataVigencia) AS DT_INI_DataVigencia \
	...
	TbFinalStep9Ini.createOrReplaceTempView("TAB_FINAL_STEP9_INI")

	TbFinalStep9Fim = spark.sql("SELECT A.MAX_DATA, \
										A.Periodo, \
										MAX(DT_FIM.DataVigencia) AS DT_FIM_DataVigencia \
									FROM TAB_FINAL_STEP8 AS A \
	...
	TbFinalStep9Fim.createOrReplaceTempView("TAB_FINAL_STEP9_FIM")

	TbFinalStep9 = spark.sql("SELECT A.DTA_REFERENCIA, \
							         A.Contrato_Operacao, \
							         A.Tipo, \
							         A.Contraparte, \
							         A.Cnpj, \
							         ...
							         DT_FIM.DT_FIM_DataVigencia \
							  FROM TAB_FINAL_STEP8 AS A \
							  LEFT JOIN TAB_FINAL_STEP9_INI AS DT_INI ON DT_INI.DataVigencia = TO_DATE(SUBSTR(A.MAX_DATA ,0,10), 'yyyy-MM-dd') AND DT_INI.DataVigencia = A.Periodo \
							  LEFT JOIN TAB_FINAL_STEP9_FIM AS DT_FIM ON DT_FIM.DataVigencia = A.Periodo AND DT_FIM.DataVigencia = TO_DATE(SUBSTR(A.MAX_DATA ,0,10), 'yyyy-MM-dd')")

	TbFinalStep9.createOrReplaceTempView("TAB_FINAL_STEP9")

	TbFinalStep11 = spark.sql("SELECT \
	...
	TbFinalStep11.write.parquet("s3://ve-datalake-analytics-vg-dev/balanco/", mode='append')
